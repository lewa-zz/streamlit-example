...
def popup(context,href):
    ...
    while _do:
        ...
        print(new_page2.url)
        _do = False
        if  new_page2.url == r"https://ygp.gdzwfw.gov.cn/zjfwcs/gd-zjcs-pub/home":
                _do = True
                new_page2.close()
    ...
    print("项目规模:%s"%amo.text_content())
    _kk = re.compile(r'\d+\.?\d+') #正则中小数点加\转义,不然.就识别任一字符
    kk = re.findall(_kk,amo.text_content())
...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_placeholder("至").fill("2022-01-15")
