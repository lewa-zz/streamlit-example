...
def table_json(table, save_one_blank=True):
    ...
    for tr in trs:
        ...
        for i in range(0, len(tds), 2):
            ...
            value = tds[i +
                        1].xpath("string(.)").extract_first(default="").strip()
...
def get_json_value(json_object, key):
    ...
    try:
        json_object = (
            isinstance(json_object, str) and get_json(
                json_object) or json_object
        )
    ...
    if isinstance(datas, list):
        for data in datas:
            keys.extend(get_all_keys(
                data, depth, current_depth=current_depth + 1))
    elif isinstance(datas, dict):
        for key, value in datas.items():
            keys.append(key)
            if isinstance(value, dict):
                keys.extend(get_all_keys(
                    value, depth, current_depth=current_depth + 1))
...
def format_time(release_time, date_format="%Y-%m-%d %H:%M:%S"):
    ...
    if "年前" in release_time:
        ...
        weeks_ago = datetime.datetime.now(
        ) - datetime.timedelta(days=int(weeks[0]) * 7)
        ...
        ) - datetime.timedelta(days=int(ndays[0]))
        ...
        ) - datetime.timedelta(hours=int(nhours[0]))
        ...
        if month and int(month[0]) <= int(get_current_date("%m")):
            ...
            release_time = str(int(get_current_date("%Y")) -
                               1) + "-" + release_time
...
def make_insert_sql(
    ...
    sql = sql.format(table=table, keys=keys,
